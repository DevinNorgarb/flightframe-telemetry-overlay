...
class GaugesConfig:
    ...
    style: str = "hud"  # hud = DJI-style digital ring; dial = classic needle
    width: int = 140
    height: int = 160
    x: int = -1
    y: int = 28
    ...
    arc_color_hex: str = "#505868"
    needle_color_hex: str = "#FF4D4F"
    tick_color_hex: str = "#FFFFFF"
    label_color_hex: str = "#C8CDDC"
    value_color_hex: str = "#FFFFFF"
...
class DashboardConfig:
    enabled: bool = False
    margin: int = 28
    speed_gauge_size: int = 220
    gforce_size: int = 110
    minimap_size: int = 180
    show_battery: bool = True
    show_classic_panel: bool = False
    start_datetime: str = ""
    label_color_hex: str = "#C8CDDC"
    value_color_hex: str = "#FFFFFF"
    muted_color_hex: str = "#AAB2C2"
    path_color_hex: str = "#00E676"
    accent_color_hex: str = "#FFD700"
...
def load_config(path: str | Path | None) -> OverlayConfig:
    ...
    merged = _merge_dict(
        {
            ...
            "dashboard": default_cfg.dashboard.__dict__,
    ...
    cfg = OverlayConfig(
        ...
        dashboard=DashboardConfig(**merged["dashboard"]),
    ...
    if cfg.gauges.enabled:
        ...
        if cfg.gauges.style not in {"hud", "dial"}:
            raise ValueError("gauges.style must be one of: hud, dial")
    ...
    if cfg.dashboard.enabled:
        if cfg.dashboard.margin < 0:
            raise ValueError("dashboard.margin must be >= 0")
        if cfg.dashboard.speed_gauge_size <= 0 or cfg.dashboard.gforce_size <= 0 or cfg.dashboard.minimap_size <= 0:
            raise ValueError("dashboard speed/gforce/minimap sizes must be > 0")
        _validate_hex_color("dashboard.label_color_hex", cfg.dashboard.label_color_hex)
        _validate_hex_color("dashboard.value_color_hex", cfg.dashboard.value_color_hex)
        _validate_hex_color("dashboard.muted_color_hex", cfg.dashboard.muted_color_hex)
        _validate_hex_color("dashboard.path_color_hex", cfg.dashboard.path_color_hex)
        _validate_hex_color("dashboard.accent_color_hex", cfg.dashboard.accent_color_hex)
